...
public class JobServiceImpl implements IJobService {
    ...
    public List<JobDto> getEmployerJobs(String employerEmail) {
        ...
        if (employer.getCompany() == null) {
            throw new RuntimeException("Employer does not assigned to the company");
        }
    ...
    public JobDto updateJobStatus(Long jobId, String status, String employerEmail) {
        ...
        if (!status.equals("ACTIVE") && !status.equals("CLOSED") && !status.equals("DRAFT")) {
            throw new RuntimeException("Job status must be ACTIVE,CLOSED, or DRAFT");
        }
        JobPortalUser employer = userRepository.findJobPortalUserByEmail(employerEmail)
                .orElseThrow(() -> new RuntimeException("Employee not found"));
        if (employer.getCompany() == null) {
        ...
        Job job = employer.getCompany().getJobs()
                .stream().filter(j -> j.getId().equals(jobId))
                .findFirst().orElseThrow(() -> new RuntimeException("Job not found"));
        job.setStatus(status);
        return ApplicationUtility.transformJobToDto(jobRepository.save(job));
    }
